...
- Full-campaign debug coverage should explicitly map to the phase plan instead of assuming late smoke helpers cover everything.
  ...
  - Transport primitive certification depends on a physical carrier near the player. If the runner cannot arrange one, record the case as `BLOCKED` with the load-command evidence instead of claiming a cargo/reward failure or mutating the asset directly. A later stronger probe should arrange and clean up a deterministic carrier before hard-passing `recover_cargo` or `deliver_supplies`.
  - `hold_area` and `clear_area` should remain WARN/not-covered until the runner samples player radius, hostile presence/absence, objective hold seconds, and cleanup over time. Do not convert these primitives to PASS with objective progress or admin completion alone.
