...
- Physical runtime probes should not silently pass when there is no controlled player entity.
  ...
  - Command-menu visual checks need a local client UI instance. `HST_CommandMenuComponent.ProbeCampaignDebugRenderedMenu()` should open the real admin tab, then count expected root widgets, visible hierarchy, and non-zero screen sizes. Treat missing `HST_CommandMenuComponent.GetLocalInstance()` as WARN/server-only evidence, not PASS.
