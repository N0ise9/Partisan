...
| --- | --- | --- |
| CRI-0: Repository truth and baseline | Complete for this checkpoint | Current code, configuration, documentation, validation, and runtime evidence are inventoried without treating broad feature presence as certification. The latest schema-47 Game-module run created the game and completed script validation; the bounded normal WorldEditor open remains the final crash-regression gate. In-process diagnostics use a development-world-only cloned-state boundary; fresh runtime proof and session-restart cleanliness remain open. |
| CRI-1: Campaign authority foundation | Implemented foundation; runtime proof pending | Schema 47 retains persisted monotonic IDs, typed command envelopes/results, bounded idempotency receipts, the resource transaction ledger, and bounded event log. Paid training, visible garrison confirmation, and player-QRF confirmation/settlement are migrated. Static validation and the latest schema-47 Game-module compile/create/script validation pass; isolated runtime and save/load certification remain pending. |
| CRI-2: Exact force manifests | Foundation complete; expansion pending | Immutable persisted manifests, expiring quotes, exact catalog validation, durable per-projection SpawnQueue authority, and the first engine-facing infantry adapter are implemented. Schema 46 makes player QRF the first paid support to freeze one authored group/all members, commit linked money/HR transactions, and enqueue the same manifest. Schema 47 adds exact member-casualty tombstones, corpse-preserving detachment, last-death cleanup, and survivor-only restart reprojection. Bounded proofs cover paid-QRF authority and force-runtime persistence/reprojection but are not runtime-executed. Vehicle/asset/multi-root execution, exact garrison realization, other paid-support migrations, and accepted settlement compaction remain open. |
