...
| `Scripts/Game/HST/Components/HST_LoadoutEditorComponent.c` | Layout root, named region binding, layout-owned left Back/ESC controls, layout-owned mode tabs, layout-owned left slot/storage rail shell and scroll hosts, layout-owned candidate/template/settings panel shell and controls, layout-owned storage add-items panel shell/grid, row layout population, preview world, dynamic preview cells, legacy factories | Mixed | Major shell regions, left controls, mode tabs, left rail chrome, candidate/template/settings panels, and the right add-items panel are layout-owned. Dynamic item previews, preview drag surface, footer hints, and volume fill are still script-populated; retire older fallback helpers as their remaining call sites move to layouts. |
...
- Loadout footer hints, preview drag surface, dynamic item preview adorners, and older fallback helpers that still create UI geometry in script.
...
- Finish replacing remaining loadout footer hints, preview drag surface, and older fallback helpers with dedicated layout widgets where practical.
