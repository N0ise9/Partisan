...
| `Scripts/Game/HST/Components/HST_MissionClientComponent.c` | Mission detail data selection, modal action dialog scaffold, notification enqueue calls, report-dialog open/close delegation | Allowed with guardrails | Mission report details now hand `HST_ReportDialogData` to the shared report dialog controller instead of creating named widgets locally. Notifications go through the shared passive toast controller. Keep future mission action/admin flows on `HST_ActionDialog.layout` instead of adding scripted panel geometry. |
