...
| `Scripts/Game/HST/Components/HST_LoadoutEditorComponent.c` | Layout root, named region binding, layout-owned left Back/ESC controls, layout-owned mode tabs, layout-owned left slot/storage rail shell and scroll hosts, layout-owned candidate/template panel shell and scroll hosts, layout-owned storage add-items panel shell/grid, row layout population, preview world, dynamic preview cells, legacy factories | Mixed | Major shell regions, left controls, mode tabs, left rail chrome, candidate/template panels, and the right add-items panel are layout-owned. Dynamic item previews and volume fill are allowed data-driven geometry. Remaining settings rows and older fallback helpers should be replaced by dedicated layout widgets/controllers. |
...
- Loadout settings rows and older fallback helpers that still create panel-control chrome in script.
...
- Finish replacing remaining loadout settings rows and older fallback helpers with dedicated layout widgets.
