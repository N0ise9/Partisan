...
| `Scripts/Game/HST/Components/HST_LoadoutEditorComponent.c` | Layout root, named region binding, layout-owned left Back/ESC controls, layout-owned mode tabs, layout-owned preview drag surface, layout-owned preview status/toast text, layout-owned left slot/storage rail shell and scroll hosts, layout-owned candidate/template/settings panel shell and controls, layout-owned footer hint slots, layout-owned storage add-items panel shell/grid, row layout population, preview world, dynamic preview cells, legacy factories | Mixed | Major shell regions, left controls, mode tabs, preview drag capture, preview status/toast text, left rail chrome, candidate/template/settings panels, footer hints, and the right add-items panel are layout-owned. Dynamic item previews and volume fill are still script-populated; retire older generic factories as their remaining call sites move to layouts. |
...
- Loadout dynamic item preview adorners, storage category tabs, and older generic factories that still create UI geometry in script.
...
- Finish replacing remaining loadout dynamic preview adorners, storage category tabs, and older generic factories with dedicated layout widgets where practical.
