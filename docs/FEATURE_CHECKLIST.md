...
| --- | --- | --- | --- | --- | --- |
| CRI-0 Truth and baseline | Complete | Complete for this checkpoint | Repository inventory, foundation validator, latest schema-46 Workbench Game compile/create/script validation, normal project-open survival, and runtime-evidence audit complete | No | Current code and documentation agree on schema 46 and one build-identity source. A native Workbench heap crash was bisected to one oversized force-authority proof and fixed by extracting and splitting it. The latest schema-46 Game-module run loaded 5,736 files/11,460 classes, created the game, and completed script validation; a separately launched normal WorldEditor project open created the game and remained responsive for every two-second observation through 20 seconds without reproducing the crash. The latest gameplay runtime artifact predates schema 43 exact-force authority, schema 44 queue authority, schema 45 physical adapter, and schema 46 paid-QRF cutover. Debug profiles fail closed outside the development world and use cloned campaign state, but a fresh runtime proof of completion, cancellation, crash recovery, and restart cleanliness is still required. |
...
| CRI-2 Force manifests | Complete for foundation | Garrison quote/confirm slice, durable SpawnQueue kernel, first exact infantry adapter, and exact player-QRF consumer | Foundation validator and latest schema-46 Game-module compile/create/script validation pass; physical HST_Dev runtime proof pending | No | Schema 46 retains immutable manifest/quote and durable per-projection queue authority, explicit active-group force/projection identity, Game Master verification evidence, and normal one-second adapter acquisition. Player QRF now issues a two-step exact quote, charges $250 plus one HR per authored member through linked transactions, and submits the same single-group manifest unchanged to SpawnQueue. Terminal failure/cancellation settles once; restored success without a live root temporarily fails closed and refunds. The adapter still supports only one infantry root plus members. Vehicle, asset, and multi-root execution, executable garrison manifests, general successful runtime reprojection, durable casualty/living-force/retirement authority, other paid-support migrations, and accepted settlement compaction remain open. The physical proof is implemented but is not engine runtime evidence until a fresh isolated HST_Dev run executes it. |
...
| Stable authority IDs and receipts | Server mutations use persisted monotonic IDs, bounded receipts, and replay-safe results. | Implemented Foundation / Needs Runtime Proof | Schema 46 persists the allocator, bounded command receipts/events, resource transactions, stable operation/garrison/quote/manifest/support IDs, exact aggregate links, request/projection/result queue identities, and explicit force/projection IDs on linked active groups. Accepted QRF confirmation replay stays linked after queue admission and after refunded settlement. Queue terminal history has pin-aware bounded retention, but accepted quote/manifest/ledger settlement history is intentionally un-compacted; add archive/tombstone retention before long-campaign certification. Fresh isolated runtime and restart evidence is still required. | Highest |
...
| Feature | Target behavior | Current status | Gap / next work | Priority |
| --- | --- | --- | --- | --- |
| Versioned save state | Durable campaign facts survive restarts and schema migration. | Implemented Foundation | Campaign schema 46 deep-copies immutable manifests, support-aware quote fields, nested roster slots, linked paid-QRF requests/transactions, durable spawn batches/slot evidence, restore epochs, stable garrison IDs, and active-group force/projection identity. Pre-schema-46 positive paid-QRF costs import as historical ledger evidence without changing balances or inventing a quote/manifest; conflicts remain warned and unaltered. `READY_FOR_HANDOFF` remains nonterminal and restores as retryable exact realization; terminal history is preserved while process-local IDs clear. The latest schema-46 Game-module compile/create/script validation passes, but process-restart behavior and general successful terminal reprojection still need runtime proof. | Highest |
...
| Exact force spawn queue | Each force projection realizes one immutable executable manifest through bounded, retryable, restore-safe work and verified registration. | Implemented Foundation / Needs Runtime Proof | Schema 46 connects player-paid QRF to the durable queue and the one-second engine adapter for exactly one infantry group root plus every frozen member. New slot success requires exact prefab, liveness, faction, native-group, Game Master, projection, and applicable seat evidence; cleanup runs asset -> member -> vehicle -> group. All exact slots first produce durable nonterminal `READY_FOR_HANDOFF`; physical-war finalization precedes `SUCCEEDED`. QRF terminal failure/cancel runs linked money/HR settlement, and setup/won/lost frames drain queue cleanup plus settlement without advancing campaign time. Vehicle/asset/multi-root execution, other paid-support migrations, general successful restore/reprojection, and the durable casualty/living-force/retirement ledger remain open. A terminal queue row is historical evidence, and the implemented physical HST_Dev proof is not runtime evidence until run. | Highest |
...
5. Run the isolated schema-43 planning, schema-44 queue authority, schema-45
   adapter, and schema-46 paid-QRF cases. Pair them with real save/restart replay
