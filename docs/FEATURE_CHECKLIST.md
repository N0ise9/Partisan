...
script-symbol references. Fresh Workbench validation and Campaign Debug/runtime
proof are pending.
...
808 script-symbol references. Its fresh Workbench and runtime gates are pending;
the preceding stamped Workbench and R31 results do not close them.
...
| --- | --- | --- | --- | --- | --- |
| CRI-0 Truth and baseline | Campaign Schema 70/runtime-settings Schema 24 is the current persisted contract; the scoped Schema-70 rebuild, exact-QRF external-restart, and preceding native-projection checkpoints remain sealed historical evidence | Current implementation `6a7943a37bd9338f176724718ec132ff108e9c82`, label `schema70-settings24-spawn-adapter-proof-backing`, adds no schema/settings migration. It recognizes staged proof backing only for the active, ready, exact reciprocal fixture graph and leaves every post-`Finish()` residue orphaned | The current tree passes Foundation at 808 references. Fresh Workbench and Campaign Debug/runtime proof are pending. R31 and its stamped Workbench result belong to preceding source `393733cc165b96ec494c72f96741cf993d400ebd` | No | Prove the current source in Workbench, then rerun the guarded targeted runtime gate. Broader restart, package, dedicated-server/client, multiplayer, reconnect/JIP, migration, unrelated suite failures, and soak gates remain open. |
...
1. Run fresh Workbench validation and the guarded targeted Campaign Debug proof
   against current source `6a7943a37bd9338f176724718ec132ff108e9c82`.
