...
| --- | --- | --- | --- | --- | --- |
| CRI-0 Truth and baseline | Complete | Complete for current source checkpoint | Repository inventory/foundation validator, stamped Workbench source validation, and packaged schema-49 evidence audit complete | No | Current code and documentation agree on campaign schema 54. The stamped Schema-54 tree identifies implementation `09a1470a4c27dbef866e8cbdba182a7df65fa027`, clean Workbench Game compile/create at 5,760 files/11,560 classes with CRC `c62de929`, and a ten-sample/20-second normal editor open. This is source/Workbench evidence only. The packaged schema-49 run proved restored stock HUD/Game Master and exposed later-repaired marker/dialog/radio/civilian defects. The latest Full Campaign Debug artifact predates schemas 43-54 and is not current certification evidence. |
| CRI-1 Authority foundation | Complete | Prior vertical slices plus schema-54 exact purchased-garrison patrol authority | Stamped Schema-54 foundation and Workbench compile/open passed; packaged runtime unavailable | No | Only newly issued policy-v2 purchased resistance garrisons gain reciprocal quote/manifest/garrison/operation/route/batch/group authority. Historical policy-v1 purchases, initial/enemy aggregate garrisons, historical patrols, and historical convoys remain on their prior contracts. Owner-change/all-dead/campaign-stop/setup or typed spawn/route-failure settlement records one no-refund receipt; packaged save/restart evidence is absent. |
| CRI-2 Force manifests | Complete for foundation | Garrison quote/confirm, durable SpawnQueue, exact infantry adapter, exact QRF/patrol consumers, and exact three-element mission convoy | Stamped source/Workbench checkpoint passed; physical and restart proof pending | No | Policy-v2 garrison quotes now freeze one non-self-populating executable root plus an arbitrary exact purchased member roster and submit it unchanged to a held SpawnQueue projection. The generic adapter remains one-root infantry-only; the convoy retains its separate PhysicalWar path. Historical/initial/enemy aggregate garrisons, garrison vehicles/assets, generic multi-root execution, and other consumers remain open. |
| CRI-3 Force runtime | Complete in source for four exact infantry operations, one exact mission convoy, and the crewless mixed-group slice | Exact casualty/reprojection, virtual/physical survivor transfer, patrol fold, convoy-element fold/rematerialization, and mixed-group cleanup | Stamped source/Workbench checkpoint passed; packaged runtime unavailable | No | Schema 54 adds an indefinitely looping local garrison patrol whose held slots remain roster authority and whose adapter validation accepts only durable survivors after first handoff. Exact garrison groups are isolated from legacy PhysicalWar activation/fold/cleanup. Real native patrol movement, casualties, fold/rematerialization, three-vehicle behavior, networking, and process restart remain unproven in a package. |
| CRI-4 Operation records | Complete for exact player/enemy infantry-QRF, enemy-patrol, purchased-garrison-patrol, and narrow mission-convoy kernels | Five explicit version-1 operation types with immutable assignment, typed materialization/route/position/settlement state, durable roster linkage, restore normalization, and fail-closed legacy isolation | Stamped Schema-54 foundation and Workbench compile/open passed; no packaged behavior run | No | Schema 54 adds `HST_OPERATION_TYPE_GARRISON_PATROL` only for new policy-v2 resistance purchases: held virtual local loop, survivor-only rematerialization, no-refund owner-change/all-dead/campaign-stop/setup or typed spawn/route-failure settlement, marker/UI projection, and `-54` quarantine. Historical policy-v1/initial/enemy aggregate garrisons stay legacy. Schema 53 enemy-patrol and earlier exact consumers retain their existing source boundaries. |
...
1. Publish the stamped schema-54 build and run a focused client/server feedback pass. Confirm that
...
11. Continue from the stamped schema-54 checkpoint with one explicitly versioned
    ...
    variants, and every other mission family remain legacy until their own cutover.
