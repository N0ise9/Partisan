...
Schema 23. Schema 63 adds canonical crew-aware combat presence and persisted
`HOT -> COOLING -> COLD` zone diagnostics with a 30-second default cooling
window. Foundation passes with 681 script-symbol references. A normal Workbench
Script Editor open compiled/created the Game module at 5,788 files/11,670 classes
with CRC `cb6475ff`, remained responsive without a crash, and produced no HST
script errors. Explicit validation passed for WORKBENCH, PC, XBOX, PS4, and PS5
with exit code `0`; all Workbench instances were closed after testing. Campaign
Debug, packaged runtime, save/restart, and multiplayer execution remain open.
...
The current development save contract is provisional Schema 63. It adds one
...
Script validation pass; Campaign Debug remains open.

Schema 63 layers on the sealed Schema-62 ownership contract. Every location has
...
| Feature | Target behavior | Current status | Gap / next work | Priority |
| --- | --- | --- | --- | --- |
| Versioned save state | Durable campaign facts survive restarts and schema migration. | Implemented Foundation / Provisional Schema-63 Source Boundary | Schema 63 persists revisioned per-zone combat-presence state and bounded contributor diagnostics while invalidating runtime-only physical samples on every restore. Pre-63 and malformed current authority become canonical `COLD` without inferring combat from vehicle/marker/group rows; valid current heat/cooling shape may survive. Schema 62 ownership receipts, backlinks, marker source revisions, and setup history remain unchanged. Deterministic fixtures remain source evidence; Foundation and explicit Workbench validation pass, while real serialization, restart/resume, reconnect, rendered UI, and JIP proof are open. | Highest |
| Runtime settings migration | Generated profile settings migrate forward without keeping obsolete setup knobs. | Implemented Foundation / Provisional Schema 23 | Schema 23 adds `capture.combatPresenceCoolingSeconds`, defaults it to 30 seconds, normalizes it to `1..300`, and gives older settings the default during migration. Schema 22's five-vehicle true-town default and Schema 21 loot-key cleanup remain intact. Workbench generation/rewrite proof is pending. | Keep |
...
1. Seal the exact provisional Schema-63 build identity, publish that exact
   package, and require the
   same `HST_BuildInfo` identity in
...
2. Runtime-prove the Schema-62 ownership boundary: every cause route, one
   ...
   activation/deactivation hysteresis. Then continue Blueprint Phase 7 with
   broader encounter and town-truth consequences.
