...
expected aggregate. This Schema-69 slice is not sealed: Foundation passes;
...
passing JUnit testcase, an empty failed list, and all exact assertions. Remaining
Workbench target configurations, Full Campaign Debug, package,
serialization/restart, live-server, network, and soak proof remain pending. The
latest sealed checkpoint remains the Schema-68 engine-planning proof described
below.
...
The latest sealed Schema 68/settings 24 enemy-planning engine-proof checkpoint is
sealed
...
Source work has entered Blueprint Phase 9 of 13 through provisional Schema 69;
the latest sealed checkpoint remains Schema 68. This is sequence position, not nine
...
| --- | --- | --- | --- | --- | --- |
| CRI-0 Truth and baseline | Campaign Schema 69/runtime-settings Schema 24 is the provisional development contract; the latest sealed checkpoint remains the Schema-68 enemy-planning engine proof at `4c9a94a1cb4811b6e75a7dca5dba70efffcb523d`, while `f97b12e` remains the prior packaged failure baseline | The Schema-68 bootstrap/profile/marker/planning work remains implemented. Schema 69 provisionally adds one exact counterattack aggregate without changing historical contract-zero rows | Schema-69 Foundation passes. Workbench PC compile/initialization log `logs_2026-07-13_15-10-44` exits `0` at CRC `c7f7a739`; focused engine log `logs_2026-07-13_15-11-13` has one passing JUnit testcase, an empty failed list, and an all-exact report | No | Execute the remaining Workbench target configurations and Full Campaign Debug Phase 17, then package-prove serialization/restart, fresh and affected-save authority, profile migration, marker behavior, dedicated/live-server, networking, and soak behavior. |
...
| CRI-2 Force manifests | Complete for the sealed foundation; provisional Schema-69 expansion | Durable SpawnQueue and exact infantry adapters retain the sealed consumers; the counterattack slice adds one frozen infantry manifest without widening vehicle, asset, or multi-root admission | Foundation, Workbench PC compile/initialization, and focused deterministic admission/roster/projection assertions pass; remaining target configurations and native/package/restart behavior remain unproved | No | Package-prove that the roster remains frozen through live casualties, virtual/physical transfer, re-entry, and restart without refill, while historical contract-zero counterattacks remain isolated. |
| CRI-3 Force runtime | Complete for the sealed consumers; provisional exact counterattack | Existing casualty/reprojection paths remain. Schema 69 adds counterattack direct travel, physical realization/fold, deterministic virtual combat, return, and survivor settlement over one durable roster | Schema-69 Campaign Debug and native/package/restart gates are pending | No | Prove live counterattack casualties, fold/root retirement, held-slot continuity, re-entry, ownership retry, return, and restart alongside every earlier family. |
| CRI-4 Operation records | Complete for sealed operation families; provisional counterattack and `PREPARED` settlement values appended | Schema 69 adds a separately typed exact counterattack and durable terminal intent without changing earlier enum values or contracts. Settlement follows prepare -> stage -> refund -> record -> finalize and resumes on restore or same-session retry | Foundation statically gates explicit and derived-ID claimant scans, and Workbench PC compile/initialization passes. The focused engine proves PREPARED pre-refund, post-refund, post-record, uncommitted-full, and physical conservative-zero recovery plus duplicate-claimant hold and foreign-execution rejection; a derived-only collision remains pending | No | Execute the remaining Workbench target configurations and the same reciprocal graph, terminal cleanup, historical isolation, current `-69` quarantine, and derived-only collision through Full Campaign Debug and packaged serialization/restart. |
...
| CRI-9c Exact enemy counterattack | Provisional Schema-69 source slice | New contract-`1` rows freeze an infantry manifest, charge exactly one attack/support pool, travel directly while virtual, preserve casualties across projection changes, resolve deterministic virtual combat, enter canonical ownership transition, return, and refund only surviving roster cost. Appended `PREPARED` terminal intent makes settlement resumable, and explicit plus derived-ID claimant scans reject ambiguous cleanup. Historical counterattacks remain contract `0`; malformed current graphs quarantine at `-69` without fabricated authority, deletion, settlement, refund, or outcome | Foundation passes; Workbench PC compile/initialization log `logs_2026-07-13_15-10-44` exits `0` at CRC `c7f7a739`; focused engine log `logs_2026-07-13_15-11-13` has one passing JUnit testcase, an empty failed list, and all exact assertions, including PREPARED crash windows and forged-open/destroyed-living/foreign-execution rejection. Remaining target configurations, Full Campaign Debug, and every external runtime gate remain open | Execute the remaining Workbench target configurations and Phase 17, then package-prove physical/virtual combat, ownership retry, return, proportional one-pool settlement, migration, quarantine idempotency, save/restart, marker continuity, multiplayer, and soak. |
...
| Durable operation state | Every force/support/order has an immutable assignment, duty state, engagement mode, materialization state, route progress where applicable, recall/return policy, and terminal result. | Sealed nine-type/eleven-consumer foundation plus provisional Schema-69 counterattack | The appended counterattack type adds direct route, deterministic combat, canonical ownership, return, and one-pool settlement over one frozen roster. The appended `PREPARED` settlement state resumes prepare/stage/refund/record/finalize across restore and same-session retry. Focused proof passes; Full Campaign Debug, packaged runtime, generalized encounters, vehicle/multi-root policy, JIP, historical missions/garrisons, and other families remain open. | Highest |
...
| Feature | Target behavior | Current status | Gap / next work | Priority |
| --- | --- | --- | --- | --- |
| Versioned save state | Durable campaign facts survive restarts and schema migration. | Implemented Foundation / Sealed Schema-68 Boundary / Provisional Schema-69 Restore Contract | Schema 69 adds no fields, preserves historical counterattacks at contract `0`, validates current exact aggregates before generic projection normalization, and resumes valid PREPARED settlement prefixes. Focused in-memory restore/replay proof passes; real serialization/restart remains open. | Highest |
...
| Enemy commander orders | Counterattacks, rebuilds, roadblocks, support calls, and HQ pressure queue durable orders. | Provisional Schema-69 Exact Counterattack / Sealed Schema-68 Planning | Periodic planning retains its sealed commitment-aware behavior. Newly admitted counterattacks provisionally use contract `1`, one frozen infantry manifest, one direct route, and one exact operation graph; historical restored counterattacks remain contract `0`. Foundation, Workbench PC compile/initialization, and focused proof pass. Execute the remaining target configurations and Full Campaign Debug, then package-prove both the new lifecycle and the earlier planning/restart gates. | Highest |
| Abstract resolution | Off-screen orders and support resolve without needing physical entities. | Provisional exact counterattack plus source-complete exact QRF/patrol routes | The counterattack slice carries the same surviving slots through direct virtual travel and deterministic virtual combat, then continues return travel after the canonical ownership result. It must not estimate a fresh roster on re-entry. Full Campaign Debug, save/restart, package, and soak proof remain open. | High |
| Physical response | Near-player enemy responses spawn, move, fight, and fold back. | Provisional Schema-69 counterattack / Needs runtime proof | Exact counterattack members materialize from the frozen roster, fold confirmed casualties back to virtual authority, and may resume strategic combat outside the bubble. Ownership publication waits for canonical transition authority, and settlement waits for return. Native spawn/contact/fold/re-entry/casualty, marker, restart, and multiplayer proof remain open. | Highest |
...
| Full Campaign Debug | One button runs a true runtime certification suite and writes structured artifacts without changing the campaign under test. | Broad Alpha / Schema-67 And Sealed Schema-68 Fixtures Wired / Focused Planning And Provisional Counterattack Autotests Passed / Needs Full-Suite And Cross-Domain Runtime Proof | Schema 67 resource assertions and 17 sealed Schema-68 planning fixtures remain wired. The provisional Schema-69 focused case `HST_TEST_EnemyCounterattackAuthority` passes in log `logs_2026-07-13_15-11-13`: one JUnit testcase, empty failed list, and an all-exact report across planning, admission, travel, combat, projection, ownership, settlement, restore, resource authority, ambiguity, quarantine, and retention. PREPARED pre-refund/post-refund/post-record/uncommitted-full/physical conservative-zero recovery passes; forged open intent, destroyed intent with a living durable roster, and a foreign execution backlink are rejected/quarantined. This did not execute Full Campaign Debug in `HST_Dev`, coordinator isolation/artifacts, live campaign authority, persistence/restart, native world behavior, package, networking, or soak. | Highest |
...
1. Execute the remaining Workbench target configurations and the provisional
   Schema-69 exact-counterattack Full Campaign Debug Phase-17 proof now that
   Foundation, the Workbench PC compile/initialization pass, and the focused
   engine case are green. Cover one-pool admission, frozen roster/route authority,
