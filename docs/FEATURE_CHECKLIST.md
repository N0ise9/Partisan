...
| --- | --- | --- | --- | --- | --- |
| CRI-0 Truth and baseline | Complete | Complete for this checkpoint | Repository inventory, foundation validator, latest schema-47 Workbench Game compile/create/script validation, normal project-open survival, and runtime-evidence audit complete | No | Current code and documentation agree on schema 47 and one build-identity source. A native Workbench heap crash was bisected to one oversized force-authority proof and fixed by extracting and splitting it. The latest schema-47 Game-module run loaded 5,737 files/11,462 classes, created the game, and completed script validation; a separate normal WorldEditor project open remained responsive at every two-second sample through 20 seconds and did not reproduce the crash. The latest gameplay runtime artifact predates schema 43 exact-force authority, schema 44 queue authority, schema 45 physical adapter, schema 46 paid-QRF cutover, and schema 47 force-runtime lifecycle. Debug profiles fail closed outside the development world and use cloned campaign state, but a fresh runtime proof of completion, cancellation, casualty cleanup, crash recovery, and restart cleanliness is still required. |
...
| CRI-2 Force manifests | Complete for foundation | Garrison quote/confirm slice, durable SpawnQueue kernel, first exact infantry adapter, exact player-QRF consumer, and first exact force-runtime lifecycle | Foundation validator and latest schema-47 Game-module compile/create/script validation pass; physical HST_Dev runtime proof pending | No | Schema 47 retains immutable manifest/quote and durable per-projection queue authority, explicit active-group force/projection identity, Game Master verification evidence, and normal one-second adapter acquisition. Player QRF issues a two-step exact quote, charges $250 plus one HR per authored member through linked transactions, and submits the same single-group manifest unchanged to SpawnQueue. Confirmed deaths retire exact member slots once, detach corpses, update durable living strength, remove the root at zero living members, and restore only surviving slots. Initial deployment failure/cancellation settles fully; post-handoff reprojection failure retains money and refunds survivors. The adapter still supports only one infantry root plus members. Vehicle, asset, and multi-root execution, executable garrison manifests, event-driven death subscription, lifecycle authority for other consumers, other paid-support migrations, and accepted settlement compaction remain open. The physical proof is implemented but is not engine runtime evidence until a fresh isolated HST_Dev run executes it. |
...
| Feature | Target behavior | Current status | Gap / next work | Priority |
| --- | --- | --- | --- | --- |
| Versioned save state | Durable campaign facts survive restarts and schema migration. | Implemented Foundation | Campaign schema 47 deep-copies immutable manifests, support-aware quote fields, nested roster/lifecycle slots, linked paid-QRF requests/transactions, successful-handoff/reprojection counters, durable spawn batches, restore epochs, stable garrison IDs, and active-group ever-populated/spawn-completed/living-force evidence. Pre-schema-46 positive paid-QRF costs import as historical ledger evidence without changing balances or inventing a quote/manifest; pre-schema-47 successful exact projections gain lifecycle evidence without invented casualties. `READY_FOR_HANDOFF` remains nonterminal and restores as retryable exact realization; successful paid-QRF history clears process IDs and requeues root plus survivors. The latest schema-47 Game-module compile/create/script validation passes, but actual process-restart behavior still needs runtime proof. | Highest |
...
5. Run the isolated schema-43 planning, schema-44 queue authority, schema-45
   adapter, schema-46 paid-QRF, and schema-47 force-runtime cases. Pair them with real save/restart replay
