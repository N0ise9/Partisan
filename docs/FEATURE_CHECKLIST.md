...
| CRI-2 Force manifests | Complete for foundation | Garrison quote/confirm vertical slice | Foundation validator and Workbench script validation pass | No | Schema 43 persists immutable manifests, expiring actor-owned quotes, typed per-slot spawn results, exact transaction links, and truthful legacy migration. The core faction group catalog records and runtime-validates exact ordered execution-prefab rosters. Visible garrison quantities 1/2/4/6/8/12 now request a server quote; confirmation submits only its ID and charges $50 plus one HR per accepted member all-or-nothing. Deterministic proof covers 1/4/7/12, HR-reservation rollback, and five interrupted-confirmation restore boundaries, but still needs a fresh isolated runtime run. Paid support, manifest-driven physicalization, terminal spawn-result retention, and accepted settlement/archive compaction remain open. |
