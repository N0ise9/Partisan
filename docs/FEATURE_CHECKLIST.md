...
on runtime-settings Schema 23. Schema 64 makes `HST_TownInfluenceRecord` the
sole political support and population authority for curated towns, retains
separate FIA/occupier/invader basis-point totals, uses strict `>8000`
resistance and `<4000` enemy ownership hysteresis, and delegates every
resulting flip to `HST_OwnershipTransitionService`. It also replaces generic
Map/War pressure with contacted-town Zone Pressure and a complete deterministic
Resistance Territory projection. Legacy support/population fields are
migration/read-only projections. Schema 64 identifies implementation
...
for that sealed checkpoint at 696 script-symbol references.
...
Phase 8 ambient-runtime slice. It adds capped global civilian actor/traffic
allocation, leased fair rotation, a four-root scheduler, asynchronous behavior-ready
admission, immutable per-zone/kind projection slots, bounded slot-specific
recovery/recycling, per-frame player-first claim observation, and a fail-closed
transient-versus-claimed vehicle persistence boundary. Foundation passes at 711
script-symbol references. Normal Workbench compilation and all-five-
configuration validation pass at 5,799 files/11,718 classes with CRC
`a6fc06df`, successful validation, zero HST script errors, and zero surviving
Workbench processes. Static source checks plus Workbench compile/validation pass;
...
| --- | --- | --- | --- | --- | --- |
| CRI-0 Truth and baseline | Campaign Schema 64/settings 23 is the last sealed checkpoint; the current Blueprint Phase 8 ambient worktree is unsealed and keeps Campaign Schema 64 while migrating settings `23 -> 24` | The last sealed identity is implementation `6f3c913eaed66926cce38b2ecafcff94084898a3`, UTC `2026-07-12T11:28:41Z`, label `schema64-canonical-town-influence`. The unsealed tree adds ambient budgeting/lifecycle/save-boundary work without inventing Campaign Schema 65 | Current Foundation passes at 711 references. Normal and all-five-configuration Workbench checks pass at 5,799 files/11,718 classes with CRC `a6fc06df`, successful validation, zero HST script errors, and zero surviving Workbench processes. Campaign Debug and packaged results remain open | No | Do not publish the unsealed tree as the old sealed identity. Package/native/save-restart/rendered-UI/ten-town soak/setup/network/reconnect/JIP certification remains independently open. |
...
| CRI-8 Civilian runtime and political consequences | Ambient foundation designed; consequence layer incomplete | Settings Schema 24 adds fair global population budgets, behavior-ready pedestrian/traffic transactions, movement recovery/recycling, owner-aware static refresh, and the transient-versus-claimed vehicle save boundary. Existing aid and ownership/security-pressure source paths remain separate from the unfinished consequence work. | Foundation passes at 711 references and normal/all-five Workbench checks pass at 5,799 files/11,718 classes with CRC `a6fc06df`; deterministic proofs are compiled but unexecuted | No | Native-prove the ten-town soak, lifecycle, capture/reset/restoration boundary, and two-nearby-same-prefab no-collapse case. Aid and ownership/security-pressure paths need runtime proof; automatic casualty, theft, nearby-combat influence, panic/recovery, and deeper local-security behavior remain open. |
...
| Runtime settings migration | Generated profile settings migrate forward without keeping obsolete setup knobs. | Implemented Foundation / Unsealed Settings Schema 24 | The `23 -> 24` migration adds normalized global civilian actor and traffic budgets, per-player additions, war-level budget penalty, runtime health/startup/stuck/recovery/backoff controls, and preserves explicit older values. Campaign persistence remains Schema 64. Generated profile rewrite and packaged migration proof are pending. | Keep |
