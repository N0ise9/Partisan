...
| Town/resource income | Income scales from town support/population and captured resources. | Broad Alpha | Economy inspection now includes next income plus source-category totals for towns, resources, factories, seaports, airfields, banks, and other owned zones, with Full Campaign Debug proof that the report matches income math; next make population scaling and resource-specific effects more visible. | High |
...
1. Repack and rerun the dedicated server proof on the r119 economy source-report
   build, then compare economy source breakdowns, icon deconflicts, Western Heights removal,
