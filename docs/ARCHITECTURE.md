...
| Certification | Campaign Schema 71 and runtime-settings Schema 24 are current. Frozen source checkpoint `5717d05af0bb1474b8e51a39ad4cef8a4fa341d6` passes Foundation at 985 references, all-target Workbench validation at 5,849 files, 12,022 classes, common CRC `312a01a9`, zero hard errors, and exact cleanup, and all five source-native focused suites at exact JUnit 91/0/0/0 with zero final engine processes. Its source-native force-authority canary also passes as scoped noncertifying evidence at 9/2/0/0/0 cases, 35/35 focused assertions, 87/87 counted conditions, 18 ordered zero-delta state rows, zero final orphans, zero unapproved diagnostics, and zero residual engine processes. Full Campaign Debug rejected this checkpoint when same-frame fixture topology changes left the global combat-presence index stale before the mission-target render-bubble baseline; fatal containment timed out at 1,087 seconds without a complete artifact, while the wrapper ended with stable source/resource-database identity and zero process or cleanup residue. The replacement source now mirrors production by refreshing combat-presence samples between the topology-changing zone case and the next baseline, keeps the global audit strict, and has a Foundation ordering guard. `7fdf3988797edeb747f5d6a6951ad0382bd93db3`, UTC `2026-07-21T19:36:22Z`, label `schema71-settings24-gate1-release-surface`, remains its embedded implementation identity. Former candidate `partisan-rc-5b1f2e98f931-20260721T193941Z`, its package hashes, its Foundation/Workbench results, and the CRI-103 pair are retained as optional historical local-candidate QA only. They are not current Gate 1 or publication authority, and generated `.pak` files are not source deliverables. | Freeze the implemented correction as a replacement source checkpoint and restart Gate 1 from Foundation. Publish only through Workbench to the Workshop after acceptance; the game downloads the addon. Historical 0e/e11/ee0/5b package-bound results do not satisfy this checkpoint. Gate 1 is failed and release remains `NO-GO`; Workshop server/client/network/JIP/reconnect, markers/UI, arbitrary migration, performance, and soak remain open. |
