...
## Current Counterattack Outbound and Dematerializing-Before-Hold Restart Boundary
...
outbound-`VIRTUAL` graph and adds exactly one production-transition-backed
dematerializing-before-strategic-hold graph. That fixture advances outbound,
crosses the production release -> `MATERIALIZING` -> `PHYSICAL` transitions,
confirms exactly one registered casualty, hands off the physical position, and
stops immediately after production begins dematerialization. `prepare` persists
that raw `DEMATERIALIZING`/`LIVE` authority instead of replacing it with the
normalized readback used for validation.

The restart carrier therefore owns two fingerprints: the raw prepared-cut
fingerprint and the post-restore normalized fingerprint. Both are nonempty and
are bound through every successful stage result; they are equal for the
outbound-`VIRTUAL` cut and distinct for the dematerializing cut. Restore performs
one normalization and one survivor reprojection into held `VIRTUAL`/strategic
authority. It preserves the exact N-1 living-slot fingerprint and one retired
casualty tombstone, and sets both group current and source positions to the
operation strategic position. Recovery advances exactly 75 m from that source;
replay restores the recovered graph as a semantic no-op. Runtime claimants and
all owned process, profile, log, spill, temporary, and guard-root residue remain
zero.
...
physicalization, live player proximity, all dematerialization states, an
interruption in `MATERIALIZING`/`PHYSICAL`, or a `PREPARED` settlement crash.
...
validation passes 5,832 files and 11,830 classes at CRC `699fab13`, reports
...
| Campaign Debug isolation | The runner deep-clones campaign state, suspends normal persistence, and restores the live state. Bounded probes additionally capture/restore the shared clock and enemy-strategic fingerprint; the coordinator holds ambient commander cadence while the clone is active. It also holds ambient local-security progression whenever the matching force-spawn worker is held, preventing a debug-only release into MATERIALIZING with no process owner. The native counterattack probe captures/restores adapter process state and keeps handoff -> `PHYSICAL` -> controlled fold/kill in production order. The separate exact restart harness is not a Campaign Debug clone: a strict startup guard authorizes one disposable fallback state carrier and one-use prepare/recover/replay stage lease, while dual raw/normalized fingerprints bind the prepared cut across processes and fail-closed cleanup removes every owned artifact. | Integrated run `seed1985_t0_p1_u1784134163` remains the latest broader Campaign Debug evidence. The separate fresh-process proof passes outbound-`VIRTUAL` and one dematerializing-before-hold cut, including N-1 tombstone continuity, once-only normalization/reprojection, 75 m recovery, replay no-op, zero runtime claimants, and exact external cleanup. `world_scope` and the wider suite stay uncertified; this synthetic production-transition-backed result must not be generalized to native physicalization, every dematerialization state, live world entities, players, delayed callbacks, caches, or native persistence selection. |
...
| Certification | Schema 70/settings 24 remains the current persisted contract. Current implementation `87a4ae2491ec5b83d37dbc43e1658f3380bb8b1c`, stamp `3a15f77`, UTC `2026-07-16T01:06:29Z`, label `schema70-settings24-counterattack-dematerializing-restart-proof`, adds no schema/settings migration. | Foundation passes at 816 references. Guarded Workbench loads 5,832 files/11,830 classes at CRC `699fab13`, reports `ScriptValidation true`, exits `0`, records zero hard errors, and cleans exactly. Fresh-process prepare/recover/replay proves outbound-`VIRTUAL` plus one production-transition-backed dematerializing-before-hold cut with dual fingerprints, exact N-1 casualty continuity, once-only normalization/reprojection, aligned strategic group positions, a 75 m continuation, replay no-op, and zero residue. This is Phase-9 proof depth, not campaign certification or a completed phase: `MATERIALIZING`/`PHYSICAL`, `PREPARED` settlement, native persistence/physicalization, world scope, package/live server-client, migration, markers/rendered UI, multiplayer/JIP/reconnect, performance, soak, and unrelated suite failures remain open. |
