...
Workbench validation and Campaign Debug/runtime proof for this source are
pending; no R31 result is attributed to it.
...
| Certification | Schema 70/settings 24 remains the current persisted contract. Current implementation `6a7943a37bd9338f176724718ec132ff108e9c82`, UTC `2026-07-15T12:54:09Z`, label `schema70-settings24-spawn-adapter-proof-backing`, adds no schema/settings migration. | The current source passes Foundation at 808 references. Fresh Workbench validation and Campaign Debug/runtime proof are pending. R31 is preceding evidence from source `393733cc165b96ec494c72f96741cf993d400ebd`: all 11 Phase-17 and both Phase-24 owner assertions pass, the 18-row state delta is exact zero, runtime error signals are zero, and external cleanup is exact. That historical full suite remains uncertified at 577 PASS/50 WARN/52 FAIL/7 BLOCKED/1 SKIPPED and 5,528/5,685 required assertions proven. Historical exact-QRF focused/external-restart evidence remains limited to that earlier checkpoint. Native persistence-source selection, world-scope, all-five Workbench, package, arrival/combat/capture/return, markers, rendered UI, performance, dedicated/live server and multiplayer/networking, reconnect/JIP, and soak gates remain open. |
