...
| Certification | Schema 68/settings 24 remains the current contract. The bootstrap/profile/marker and commitment-aware planning corrections are sealed as source/Workbench evidence. A focused command-line engine autotest additionally proves all 17 deterministic planning fixtures, including retry quarantine repeated-pass idempotency, without claiming HST_Dev, coordinator, world, persistence, restart, package, or network proof. The latest packaged baseline created the canonical profile root and exposed fresh enemy authority quarantine plus 598 repeated warnings; it did not contain a retired tree. | Foundation 753 and final stamped-tree all-target Workbench CRC `5a998c21` pass for implementation `4c9a94a1cb4811b6e75a7dca5dba70efffcb523d`; focused engine log `logs_2026-07-13_11-44-28` records one JUnit testcase with no failure, an empty failed list, and `AllExact()` true across all 17 fixtures. Execute the matching planning assertions in Full Campaign Debug in `HST_Dev` with coordinator isolation/artifacts and live-authority observation, then package-prove fresh start, affected-save recovery/restart, retired-tree removal and file/directory conflict archival, protected/player marker behavior, rendered UI, performance, dedicated and live server, multiplayer/networking, reconnect, JIP, and soak behavior. |
