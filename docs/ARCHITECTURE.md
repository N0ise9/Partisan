...
## Provisional Schema 69 / Settings 24 Exact Enemy Counterattack

The active development tree provisionally advances Campaign Schema 69 while
runtime settings remain on Schema 24. A newly admitted enemy counterattack now
...
roster, route, debit, refund, settlement, or ownership outcome. This slice is
not sealed. Foundation passes. Workbench PC compile/initialization log
`logs_2026-07-13_15-10-44` exits `0` with Game CRC `c7f7a739`. Focused engine log
`logs_2026-07-13_15-11-13` records one passing JUnit testcase, an empty failed
list, and an all-exact report, including PREPARED pre-refund, post-refund,
post-record, uncommitted-full, and physical conservative-zero recovery plus
forged-open, destroyed-with-living-roster, and foreign-execution rejection.
Remaining Workbench target configurations, Full Campaign Debug,
serialization/restart, package, live-server, network, and soak proof remain
open. The latest sealed checkpoint remains the Schema-68 engine-planning proof
recorded below.
...
| Exact enemy counterattack | Provisional Schema 69 gives new contract-`1` counterattacks one reciprocal order/operation/manifest/batch/group aggregate, one charged attack or support pool, direct virtual travel, deterministic combat, physical/virtual casualty continuity, canonical ownership transition, return, and proportional survivor refund. The appended `PREPARED` settlement intent enforces prepare, stage, refund, record, and finalize ordering across restore and same-session retry; derived-ID claimant scans reject ambiguous residue. Historical rows remain contract `0`; malformed current graphs quarantine at `-69` without fabrication, deletion, settlement, refund, or outcome. | Foundation, the Workbench PC compile/initialization pass, and the focused exact-counterattack engine case pass. Execute the remaining Workbench target configurations, Full Campaign Debug Phase 17, serialization/restart, native projection/combat, package, live-server, networking, and soak proof before sealing the schema. |
...
| Certification | Schema 69/settings 24 is the provisional development contract; Schema 68 remains the latest sealed checkpoint. Schema-69 Foundation passes, the Workbench PC compile/initialization pass exits `0` at CRC `c7f7a739`, and focused engine log `logs_2026-07-13_15-11-13` has one passing JUnit testcase, an empty failed list, and an all-exact report. | Execute the remaining Workbench target configurations and Full Campaign Debug Phase 17 before stamping the new tree. Then package-prove serialization/restart, physical/virtual combat, canonical ownership and one-pool settlement, profile migration, markers, rendered UI, performance, dedicated/live server, multiplayer/networking, reconnect, JIP, and soak behavior. |
