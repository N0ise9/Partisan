...
| Campaign Debug isolation | The runner deep-clones campaign state, suspends normal persistence, and restores the live state. Bounded probes additionally capture/restore the shared clock and enemy-strategic fingerprint; the coordinator holds ambient commander cadence while the clone is active. It also holds ambient local-security progression whenever the matching force-spawn worker is held, preventing a debug-only release into MATERIALIZING with no process owner. The native counterattack probe captures/restores adapter process state, keeps successful handoff -> `PHYSICAL` -> controlled fold/kill in one coordinator invocation, yields for asynchronous death/cleanup/re-entry, uses typed cleanup before fail-closed dual-key focal cleanup, explicitly retires orphan entities, and treats a missing tracked exact order as fatal: record failure, stop, restore, save artifacts, and skip later telemetry. Staged spawn-adapter backing now exists only while the proof is active and ready and its exact cancel/success/failure identities, reciprocal group/manifest/batch, recomputed frozen-manifest hash, and request/operation/manifest/result/force/projection/faction/prefab/policy fields all agree. Every post-case probe still runs, and post-`Finish()` residue remains orphaned. | Preceding R31 passes all 11 Phase-17 targets and both Phase-24 owner assertions on source `393733cc165b96ec494c72f96741cf993d400ebd`, with initial/casualty/survivor physical settlement and casualty death each observed at 1/4. It classifies 16/16 Phase-24 owners with two open exact projections and restores all 18 tracked state deltas to zero. This is historical proof of the corrected ordering in the guarded runtime scope. The current proof-backing source passes Foundation at 808 and stamped PC Game validation at CRC `dc565606`; Campaign Debug/runtime proof is pending. R26 and R28b remain historical positive boundaries for exact QRF and the earlier exact-counterattack casualty cycle respectively. World entities, players, delayed callbacks, caches, and real process restart remain outside the clone boundary; `world_scope` stays BLOCKED. |
...
| Certification | Schema 70/settings 24 remains the current persisted contract. Current implementation `6a7943a37bd9338f176724718ec132ff108e9c82`, stamp `40c2d48`, UTC `2026-07-15T12:54:09Z`, label `schema70-settings24-spawn-adapter-proof-backing`, adds no schema/settings migration. | The current source passes Foundation at 808 references. Stamped PC Game Module validation uses project `histasi`/GUID `698532771130111D`, loads 5,830 files/11,822 classes at CRC `dc565606`, reaches `Game created`, reports `Script validation successful`, exits `0`, records zero hard errors, and leaves cleanup guard/process/default residue/external spill at zero. Campaign Debug/runtime proof remains pending. R31 is preceding evidence from source `393733cc165b96ec494c72f96741cf993d400ebd`: all 11 Phase-17 and both Phase-24 owner assertions pass, the 18-row state delta is exact zero, runtime error signals are zero, and external cleanup is exact. That historical full suite remains uncertified at 577 PASS/50 WARN/52 FAIL/7 BLOCKED/1 SKIPPED and 5,528/5,685 required assertions proven. Historical exact-QRF focused/external-restart evidence remains limited to that earlier checkpoint. Native persistence-source selection, world-scope, all-five Workbench, package, arrival/combat/capture/return, markers, rendered UI, performance, dedicated/live server and multiplayer/networking, reconnect/JIP, and soak gates remain open. |
