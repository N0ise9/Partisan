...
The current `Run Campaign Debug` implementation is a useful certification scaffold. It is no longer just a string/report smoke runner: it has typed results, deterministic debug run prefixes, artifacts, status/cancel/cleanup commands, stronger bootstrap/preflight/HQ/economy assertions, convoy readiness/progress probing, a POW captive free/follow probe, seeded persistence smoke roundtrip assertions, Phase 24 pacing/end assertions, and mission cleanup checks.
...
- Early mechanics are still partly report/action wrappers. Phase 12 persistence, civilian aid, support cancellation, and captive follow now have typed assertions, but generated content, zone activation spawn/cleanup, civilian aid bounds, support cancellation cleanup, garage/vehicle/loadout action tests, and UI coverage are not fully upgraded to hard typed assertions.
