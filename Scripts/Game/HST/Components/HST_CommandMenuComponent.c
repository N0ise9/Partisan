...
class HST_CommandMenuComponent : ScriptComponent
{
	...
	bool ProbeCampaignDebugRenderedMenu(out string actual)
	{
		actual = "not executed";
		...
		OpenMenuToTab("admin", "campaign debug rendered menu probe");
		Widget root = m_wMenuRoot;
		bool rootReady = root && root.IsVisibleInHierarchy();
		int expectedCount;
		int foundCount;
		int visibleCount;
		int sizedCount;
		string missingWidgets;
		string hiddenWidgets;
		string unsizedWidgets;
		array<string> expectedWidgets = {};
		expectedWidgets.Insert("HST_CommandMenuRoot");
		expectedWidgets.Insert("ScreenDimmer");
		expectedWidgets.Insert("CommandSurface");
		expectedWidgets.Insert("Header");
		expectedWidgets.Insert("NavigationPanel");
		expectedWidgets.Insert("TabItems");
		expectedWidgets.Insert("StatsPanel");
		expectedWidgets.Insert("MainPanel");
		expectedWidgets.Insert("MainItems");
		expectedWidgets.Insert("ActivityPanel");
		expectedWidgets.Insert("ActionsPanel");
		expectedWidgets.Insert("ActionsItems");
		expectedWidgets.Insert("CloseButton");
		expectedWidgets.Insert("CloseLabel");

		foreach (string widgetName : expectedWidgets)
		{
			expectedCount++;
			Widget probeWidget = FindMenuWidget(root, widgetName);
			if (!probeWidget)
			{
				missingWidgets = AppendCommandMenuDebugCsv(missingWidgets, widgetName);
				continue;
			}

			foundCount++;
			if (probeWidget.IsVisibleInHierarchy())
				visibleCount++;
			else
				hiddenWidgets = AppendCommandMenuDebugCsv(hiddenWidgets, widgetName);

			float probeWidth;
			float probeHeight;
			probeWidget.GetScreenSize(probeWidth, probeHeight);
			if (probeWidth >= 1.0 && probeHeight >= 1.0)
				sizedCount++;
			else
				unsizedWidgets = AppendCommandMenuDebugCsv(unsizedWidgets, string.Format("%1:%2x%3", widgetName, Math.Round(probeWidth), Math.Round(probeHeight)));
		}

		bool ready = rootReady && expectedCount > 0 && foundCount == expectedCount && visibleCount == expectedCount && sizedCount == expectedCount;
		actual = string.Format("was open %1 | now open %2 | root ready %3", wasOpen, m_bMenuOpen, rootReady);
		actual = actual + string.Format(" | widgets %1/%2 | visible %3 | sized %4", foundCount, expectedCount, visibleCount, sizedCount);
		actual = actual + " | missing " + EmptyCommandMenuDebugCsv(missingWidgets);
		actual = actual + " | hidden " + EmptyCommandMenuDebugCsv(hiddenWidgets);
		actual = actual + " | unsized " + EmptyCommandMenuDebugCsv(unsizedWidgets);
		actual = actual + " | root " + HST_UIDebug.WidgetSummary(root);
		return ready;
	}

	protected string AppendCommandMenuDebugCsv(string current, string value)
	{
		if (value.IsEmpty())
			return current;
		if (current.IsEmpty())
	...
	protected string EmptyCommandMenuDebugCsv(string value)
	{
		if (value.IsEmpty())
			return "none";
